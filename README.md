...
| `get_messages` | `chat_id`, `limit` (default: 20) | Последние сообщения из чата (голосовые, видеосообщения и аудио показываются с длительностью) |
| `send_message` | `chat_id`, `text` | Отправить сообщение в чат |
| `download_voice_message` | `chat_id`, `message_id` | Скачать голосовое/видео/аудио сообщение по ID. Файл сохраняется в `downloads/` |
...
> Скачай голосовое сообщение 42 из чата 123456789
```
...
├── config.py          # Загрузка TG_API_ID, TG_API_HASH, пути к сессии и downloads
├── requirements.txt   # Зависимости
├── .env               # API ключи (не коммитится)
├── .gitignore
├── sessions/          # Файлы сессии Telethon (не коммитится)
└── downloads/         # Скачанные медиафайлы (не коммитится)
```
